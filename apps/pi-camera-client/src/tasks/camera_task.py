    ...
    while True:
        try:
            ...
            with Camera(debug_ns=debug_ns) as camera:
                logger.info(
                    'Initialized Camera module. Started loop of capturing camera data')
                while True:
                    ...
                    if len(video_frames) > 0:
                        try:
                            if outgoing_video_chunk_queue.full():
                                logger.warning(
                                    'The video queue is full, remove oldest chunk')
            ...
            logger.exception(
                'Camera module has fatal error, re-initializing it after 2s')
